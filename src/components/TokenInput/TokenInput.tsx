    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="rounded-lg bg-[#130F25] border border-[#2B2649] h-11 flex flex-nowrap flex-row items-center gap-3">
        <div className="px-3 py-2 ">
          <input
            type="number"
            className=" text-white w-full bg-[#130F25] outline-none"
            value={`${value}`}
            onChange={handleChange}
          />
        </div>
        <div
          className="px-3 py-2 border-l-2 border-[#2B2649] h-11 w-full flex flex-row justify-between"
        ...
        >
          ...
          <span className="flex flex-row text-white items-center select-none">
            {showMax && token?.balance ? (
              <small className=" text-white" onClick={handleMaxClick}>
                MAX
              </small>
            ) : (
              ""
